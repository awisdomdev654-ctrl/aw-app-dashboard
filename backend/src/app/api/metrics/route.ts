...
export async function GET() {
    ...
    const awaitingProducer = stems.filter(s => s.status === 'awaiting_review').length
    const pendingUpload = stems.filter(s => s.status === 'pending_upload').length
    const signedActive = stems.filter(s => s.status === 'signed_url_active').length
    ...
    const highPrioritySessions = pendingUpload + signedActive + awaitingProducer
